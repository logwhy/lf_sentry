...
void StandardRobotPpRos2Node::createSubscription()
{
  ...
  cmd_sentry_sub_ = this->create_subscription<pb_rm_interfaces::msg::SentryCmd>(
    "sentry_cmd", 10,  // Topic name
    std::bind(&StandardRobotPpRos2Node::cmdSentryCallback, this, std::placeholders::_1));
}
...
void StandardRobotPpRos2Node::publishRfidStatus(ReceiveRfidStatus & rfid_status)
{
  ...
  const uint32_t s1 = rfid_status.data.rfid_status;
  const uint8_t s2 = rfid_status.data.rfid_status_2;

  auto bit32 = [&](int bit) -> bool {
    return ((s1 >> bit) & 0x1U) != 0U;
  };

  auto bit8 = [&](int bit) -> bool {
    return ((s2 >> bit) & 0x1U) != 0U;
  };
  ...
  // bit 0~4
  msg.base_gain_point = bit32(0);                              // 己方基地增益点
  msg.central_highland_gain_point = bit32(1);                  // 己方环形高地增益点
  msg.enemy_central_highland_gain_point = bit32(2);            // 对方环形高地增益点
  msg.friendly_trapezoidal_highland_gain_point = bit32(3);     // 己方梯形高地增益点
  msg.enemy_trapezoidal_highland_gain_point = bit32(4);        // 对方梯形高地增益点

  // bit 5~8
  msg.friendly_fly_ramp_front_gain_point = bit32(5);           // 己方飞坡前
  msg.friendly_fly_ramp_back_gain_point = bit32(6);            // 己方飞坡后
  msg.enemy_fly_ramp_front_gain_point = bit32(7);              // 对方飞坡前
  msg.enemy_fly_ramp_back_gain_point = bit32(8);               // 对方飞坡后

  // bit 9~12
  msg.friendly_central_highland_lower_gain_point = bit32(9);   // 己方中央高地下方
  msg.friendly_central_highland_upper_gain_point = bit32(10);  // 己方中央高地上方
  msg.enemy_central_highland_lower_gain_point = bit32(11);     // 对方中央高地下方
  msg.enemy_central_highland_upper_gain_point = bit32(12);     // 对方中央高地上方

  // bit 13~16
  msg.friendly_highway_lower_gain_point = bit32(13);           // 己方公路下方
  msg.friendly_highway_upper_gain_point = bit32(14);           // 己方公路上方
  msg.enemy_highway_lower_gain_point = bit32(15);              // 对方公路下方
  msg.enemy_highway_upper_gain_point = bit32(16);              // 对方公路上方

  // bit 17~23
  msg.friendly_fortress_gain_point = bit32(17);                // 己方堡垒增益点
  msg.friendly_outpost_gain_point = bit32(18);                 // 己方前哨站增益点

  // RMUL 重点字段
  msg.friendly_supply_zone_non_exchange = bit32(19);           // 己方与资源区不重叠的补给区（RMUL 补给区）
  msg.friendly_supply_zone_exchange = bit32(20);               // 己方与资源区重叠的补给区
  msg.center_gain_point = bit32(23);                           // 中心增益点（仅 RMUL 适用）

  // 兼容旧字段名：老接口里叫 big_resource_island，
  // 新协议图里 bit21/22 更接近“装配增益点”
  // 为了不动上层逻辑，先继续复用旧字段
  msg.friendly_big_resource_island = bit32(21);                // 己方装配增益点 -> 兼容映射
  msg.enemy_big_resource_island = bit32(22);                   // 对方装配增益点 -> 兼容映射

  // =========================
  // rfid_status_2（当前老消息未定义对应字段）
  // 如后续需要，可继续扩展：
  // bit8  0: 对方隧道下方
  // bit8  1: 对方隧道中间
  // bit8  2: 对方隧道上方
...
void StandardRobotPpRos2Node::publishSentryInfo(ReceiveSentryInfoData & sentry_info)
{
auto msg = pb_rm_interfaces::msg::SentryInfo();
  
  // sentry_info (uint32) 位域解析
  uint32_t info = sentry_info.data.sentry_info;
  msg.exchanged_projectile_allowance = info & 0x7FF;          // bit 0-10
  msg.remote_projectile_exchange_count = (info >> 11) & 0xF;  // bit 11-14
  msg.remote_hp_exchange_count = (info >> 15) & 0xF;          // bit 15-18
  msg.can_free_respawn = (info >> 19) & 0x1;                  // bit 19
  msg.can_pay_respawn = (info >> 20) & 0x1;                   // bit 20
  msg.respawn_gold_cost = (info >> 21) & 0x3FF;               // bit 21-30

  // sentry_info_2 (uint16) 位域解析
  uint16_t info2 = sentry_info.data.sentry_info_2;
  msg.current_posture = (info2 >> 12) & 0x3;                  // bit 12-13
...
void StandardRobotPpRos2Node::sendData()
{
  ...
  while (rclcpp::ok()) {
    ...
    try {
      ...
      // 200 / 7 ≈ 28.5Hz，所以每 7 次循环发送一次 SentryCmd。
      sentry_send_counter++;
      if (sentry_send_counter >= 7) {
        sentry_send_counter = 0;
        crc16::append_CRC16_check_sum(
          reinterpret_cast<uint8_t *>(&send_sentry_cmd_data_), sizeof(SendSentryCmdData));
        std::vector<uint8_t> send_sentry_data = toVector(send_sentry_cmd_data_);
        serial_driver_->port()->send(send_sentry_data);
      }
...
void StandardRobotPpRos2Node::cmdSentryCallback(
    const pb_rm_interfaces::msg::SentryCmd::SharedPtr msg)
{
    ...
    // bit 0     : 是否确认免费复活
    // bit 1     : 是否确认兑换立即复活
    // bit 2-12  : 将要兑换的发弹量值（11 bit）
    // bit 13-16 : 远程兑换发弹量请求次数（4 bit）
    // bit 17-20 : 远程兑换血量请求次数（4 bit）
    // bit 21-22 : 哨兵姿态指令（1=进攻，2=防御，3=移动）
    // bit 23    : 是否确认激活能量机关
    // bit 24-31 : 保留位，置 0
    // ------------------------------------------------------------
    ...
    // 0 = 不确认复活
    // 1 = 确认复活
    const uint32_t confirm_respawn = static_cast<uint32_t>(msg->confirm_respawn);
    cmd |= (confirm_respawn & 0x1U) << 0;

    // bit 1：确认兑换立即复活
    // 0 = 不兑换
    // 1 = 确认兑换
    const uint32_t confirm_pay_respawn =
        static_cast<uint32_t>(msg->confirm_pay_respawn);
    cmd |= (confirm_pay_respawn & 0x1U) << 1;

    // bit 2-12：将要兑换的发弹量值
    // 协议只给 11 bit，因此这里取低 11 位
    // 注意：该值在业务上要求“单调递增”
    const uint32_t projectile_allowance =
        static_cast<uint32_t>(msg->projectile_allowance_to_exchange);
    cmd |= (projectile_allowance & 0x7FFU) << 2;

    // bit 13-16：远程兑换发弹量请求次数
    // 协议只给 4 bit，因此这里取低 4 位
    // 注意：该值在业务上要求“单调递增且每次只能 +1”
    const uint32_t remote_projectile_req_count =
        static_cast<uint32_t>(msg->remote_projectile_request_count);
    cmd |= (remote_projectile_req_count & 0x0FU) << 13;

    // bit 17-20：远程兑换血量请求次数
    // 协议只给 4 bit，因此这里取低 4 位
    // 注意：该值在业务上要求“单调递增且每次只能 +1”
    const uint32_t remote_hp_req_count =
        static_cast<uint32_t>(msg->remote_hp_request_count);
    cmd |= (remote_hp_req_count & 0x0FU) << 17;
    ...
    // 3 = 移动姿态
    // 若收到非法值，则回退到默认移动姿态
    uint32_t posture_command = static_cast<uint32_t>(msg->posture_command);
    if (posture_command < 1U || posture_command > 3U) {
        posture_command = 3U;
    }
    cmd |= (posture_command & 0x03U) << 21;

    // bit 23：确认激活能量机关
    // 0 = 不确认
    // 1 = 确认激活
    const uint32_t activate_rune = static_cast<uint32_t>(msg->activate_rune);
    cmd |= (activate_rune & 0x1U) << 23;

    // bit 24-31：保留位
    // 当前协议要求保留为 0，因此无需额外处理

    send_sentry_cmd_data_.data.sentry_cmd = cmd;
}
}  // namespace standard_robot_pp_ros2
