...
namespace io
{
HikRobot::HikRobot(double exposure_ms, double gain, const std::string & id)
: id_(id),  // <--- 關鍵：新增這行，將參數 id 存入成員變數 id_
...
void HikRobot::capture_start()
{
  ...
  for (unsigned int i = 0; i < device_list.nDeviceNum; ++i) {
    auto* pDeviceInfo = device_list.pDeviceInfo[i];
    // 获取当前枚举到的相机序列号
    std::string current_sn = reinterpret_cast<char*>(pDeviceInfo->SpecialInfo.stUsb3VInfo.chSerialNumber);
    
    // 如果序列号匹配，或者是旧版逻辑中只有一台相机且 id 包含 ':'
    if (current_sn == id_ || id_.find(':') != std::string::npos) {
      target_index = i;
      
      // 顺便更新 vid/pid，这样即使 SNCamera 没传 vid_pid，reset_usb 也能工作
      vid_ = pDeviceInfo->SpecialInfo.stUsb3VInfo.idVendor;
      pid_ = pDeviceInfo->SpecialInfo.stUsb3VInfo.idProduct;
      break;
    }
  }

  if (target_index == -1) {
    tools::logger()->error("找不到标识符为 {} 的相机!", id_);
    return;
  }
